...
public class TopKVector<T extends Comparable<T>> {
    ...
    public void updateWithNewElement(T currentElement) {
        if (localTopK.size() < k) {
            /*
             * There are less than k objects in localTopK.
             * Add the current element at the end of localTopK.
             */
            ...
            if (currentElement.compareTo(localTopK.elementAt(k - 1)) > 0) {
                /*
                 * The current element is better than the least object in localTopK.
                 * Substitute the last object of localTopK with the current object.
                 */
    ...
    private void sortAfterInsertNewElement() {
        /*
         * The last object is the only one that is potentially not in the right position.
         */
        T swap;
