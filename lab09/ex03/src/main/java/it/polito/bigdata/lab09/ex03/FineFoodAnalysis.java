package it.polito.bigdata.lab09.ex03;
...
public class FineFoodAnalysis {
    ...
    public static void main(String[] args) {
        ...
        SparkSession ss = SparkSession.builder()
            .appName("Lab 9.3 - Classification on the Amazon fine-foods dataset (multiple features)")
            .getOrCreate();
        ...
        JavaRDD<LabeledPoint> labeledPointsRDD = reviewsDataframe
            ...
            .map(r -> {
                // Get the review text, summary and score
                String text = r.getString(9);
                String summary = r.getString(8);
                Integer score = r.getInt(6);
                
                // Compute measures
                Integer length = text.length();
                Integer numberOfWords = text.split(" ").length;
                Integer summaryLength = summary.length();
                Integer summaryNumberOfWords = summary.split(" ").length;

                // Compute the helpfulness index
                Double helpfulness = (double) r.getInt(4) / r.getInt(5);

                // Emit the labeled point
                return new LabeledPoint(helpfulness < THRESHOLD ? 0.0 : 1.0, Vectors.dense(
                    length, numberOfWords, summaryLength, summaryNumberOfWords, score));
            });
